...
def get_approx_star_field(img):
    ...
    img_stars = np.array(img_stars, dtype='float64')
    return img_stars
...
def convert_pix_to_hpr(x, y, himap):
    ...
    coord = himap.pixel_to_world(x, y)
    el, pa = convert_hpc_to_hpr(coord.Tx, coord.Ty)
    return el, pa
...
def convert_hpr_to_pix(el, pa, himap):
    ...
    coord = SkyCoord(lon, lat, frame=himap.coordinate_frame)
    x, y = himap.world_to_pixel(coord)
    return x, y
